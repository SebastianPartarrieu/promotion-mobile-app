...
def post_client_info():
    ...
    try:  # catch the exception if the client already exists in the database
        res = db.post_client_info(
            clnom=clnom, clpnom=clpnom, clemail=clemail, aid=aid, clmdp=clmdp)
        p = int(res[0][0])
...
def post_commerce_info():
    ...
    url_ext, code_postal = PARAMS.get("url_ext", None), int(
        PARAMS.get("code_postal", None))
    rue_and_num = PARAMS.get("rue_and_num", None)
...
def upload_image(pid):
    ...
    uploaded_file.save(os.path.join(
        app.config['UPLOAD_PATH_PROMOTION'], filename))
...
def delete_images(pid):
    ...
    for imgname in res:
        os.remove(os.path.join(
            app.config['UPLOAD_PATH_PROMOTION'], imgname[0]))
    return "", 204
